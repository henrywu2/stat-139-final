...
```{r}
turnout_data <- read.csv("../data/processed/data.csv")
head(turnout_data)
```
...
hist(turnout_data$turnout.rate, main = 'Histogram of Turnout Rate', xlab = 'Turnout Rate')
```
...
hist(turnout_data$gsmn_math_g3_2013, main = 'Histogram of 2013 Mean 3rd Grade Math Scores', xlab = 'Mean Grade')
```
...
```{r poverty}
plot(turnout_data$poor_share2010, turnout_data$turnout.rate, main = 'Turnout Rate vs. Poverty Rate', xlab = '2010 Poverty Rate', ylab = '2020 Turnout Rate')
```
